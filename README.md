...
2. Соберите и запустите сервисы:
   ...
   через переменные окружения `ETCD_USE_EMBEDDED`, `MINIO_USE_EMBEDDED`
   и `PULSAR_USE_EMBEDDED`. Это исключает зависимость от внешних сервисов и
   предотвращает ошибки вида `find no available datacoord` при старте контейнера.
