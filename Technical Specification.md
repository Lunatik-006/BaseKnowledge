...
- **Платформа:** Telegram Bot + **Telegram Mini App (WebApp)**  
  > фронтенд — **Next.js**; бэкенд — **Python (FastAPI)**.
- **LLM через Replicate:**  
...
**system prompt for gpt 5-structured**  
```
...
Задача: верни JSON по схеме. Если текста мало — верни один или несколько инсайтов, которые можно выделить из текста.
...
Ты — куратор-редактор. Сгруппируй список инсайтов по 10–30 темам.
Верни JSON:
{
"topics":\[
